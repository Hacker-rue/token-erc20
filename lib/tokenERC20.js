...
class TokenERC20Contract extends Contract {
    ...
    */
    async TotalSupply(ctx) {
        ...
        const totalSupply = totalSupplyBytes.toString()
        return totalSupply;
    }
    ...
    async BalanceOf(ctx, owner) {
        ...
        const balance = balanceBytes.toString()
    ...
    */
    async TransferFrom(ctx, from, to, value) {
        ...
        const currentAllowance = BigInt(currentAllowanceBytes.toString())
    ...
    async _transfer(ctx, from, to, value) {
        ...
        if (valueInt < 0n) { // transfer of 0 is allowed in ERC20, so just validate against negative amounts
        ...
        const fromCurrentBalance = BigInt(fromCurrentBalanceBytes.toString());
        ...
        if (!toCurrentBalanceBytes || toCurrentBalanceBytes.length === 0) {
            ...
            toCurrentBalance = BigInt(toCurrentBalanceBytes.toString());
        }
    ...
    async Allowance(ctx, owner, spender) {
        ...
        const allowance = allowanceBytes.toString();
        return allowance;
    }
    ...
    async Mint(ctx, amount) {
        ...
        if (!currentBalanceBytes || currentBalanceBytes.length === 0) {
            ...
            currentBalance = BigInt(currentBalanceBytes.toString());
        }
        ...
        if (!totalSupplyBytes || totalSupplyBytes.length === 0) {
            ...
            totalSupply = BigInt(totalSupplyBytes.toString());
        }
    ...
    async Burn(ctx, amount) {
        ...
        const currentBalance = BigInt(currentBalanceBytes.toString());
        ...
        const totalSupply = this.sub(BigInt(totalSupplyBytes.toString()), amountInt);
        await ctx.stub.putState(totalSupplyKey, Buffer.from(totalSupply.toString()));
